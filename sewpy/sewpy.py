...
class SEW():
    ...
    def __init__(self,
        ...
        if workdir is not None:
            ...
            if os.path.isdir(workdir):
                ...
                logger.info(
                    "Making new SExtractor workdir '%s'..." % (workdir))
                os.makedirs(workdir)
        else:
    ...
    def get_version(self):
		...
		"""
        try:
            p = subprocess.Popen([self.sexpath],
                                 stdout=subprocess.PIPE,
                                 stderr=subprocess.PIPE)
        except:
            raise RuntimeError(
                "Could not run SExtractor. Is the path '%s' correct ? If not, specify sexpath='/path/to/sextractor'"
                % self.sexpath)
        out, err = p.communicate()
        version_match = re.search("[Vv]ersion ([0-9\.])+",
                                  err.decode(encoding='UTF-8'))
        if version_match is False:
            raise RuntimeError(
                "Could not determine SExctractor version, check the output of running '%s'"
                % (self.sexpath))
    ...
    def _write_default_config(self, force=False):
        ...
        if force or not os.path.exists(self._get_config_filepath()):
            p = subprocess.Popen([self.sexpath, "-dd"],
                                 stdout=subprocess.PIPE,
                                 stderr=subprocess.PIPE)
            out, err = p.communicate()
            if err != "":
...
1 2 1""")
    ...
    def _clean_workdir(self):
        ...
        toremove = [
            self._get_config_filepath(),
            self._get_params_filepath(),
            self._get_conv_filepath(),
            self._get_psf_filepath()
        ]
    ...
    def _write_assoc(self, cat, xname, yname, imgname):
        ...
        f.close()
        logger.debug(
            "Wrote ASSOC file %s..." % (self._get_assoc_filepath(imgname)))
    ...
    def _add_prefix(self, table, prefix):
        ...
        for colname in table.colnames:
            if colname not in [
                    "VECTOR_ASSOC", "VECTOR_ASSOC_1", "VECTOR_ASSOC_2"
            ]:
    ...
    def __call__(self,
        ...
        # And we run
        logger.info(
            "Starting SExtractor now, with niceness %s..." % (self.nice))
        ...
        output = {
            "catfilepath": self._get_cat_filepath(imgname),
        ...
        if returncat:
            if assoc_cat is None:
                ...
                if len(duplassoc) > 0:
                    logger.warning(
                        "%i sources from the SExtractor catalog are strange duplicates (bug ?), I discard them."
                        % (len(duplassoc)))
                ...
                if not len(intable) == len(joined):
                    raise RuntimeError(
                        ...
                        % (len(intable), len(joined), intable.colnames,
                           joined.colnames))
                ...
                if joined.masked:
                    ...
                    joined[prefix + "assoc_flag"] = joined[
                        joined.colnames[-1]].mask == False
...
"""
    ...
    fullparamlist = list(
        map(lambda s: s[1:-1],
            re.compile("#\w*\s").findall(fullparamtxt)))
